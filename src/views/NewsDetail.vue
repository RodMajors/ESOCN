...
onMounted(async () => {
  try {
    ...
    // 加载 HTML 内容
    const htmlUrl = import.meta.env.DEV ? `/src/assets/news/${newsId}.html` : `/assets/news/${newsId}.html`;
    console.log('Fetching HTML from:', htmlUrl);
    const htmlResponse = await fetch(htmlUrl);
    if (!htmlResponse.ok) throw new Error(`Failed to load news content: ${htmlResponse.status} ${htmlResponse.statusText}`);
    ...
    if (!blogBody) {
      console.warn('blog-body-wrap not found, trying alternative selectors');
      blogBody = doc.querySelector('div[class*="blog-body-wrap"]') || doc.querySelector('body > div');
      if (blogBody) {
