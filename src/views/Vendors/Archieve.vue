<template>
    <header
        class="vendor-header"
        :style="{ backgroundImage: `url(/background/vendor/Infinite_Archive.png)` }"
    >
        <div class="title-overlay">
            <h1>无尽档案塔商人</h1>
            <p class="en-title">INFINITE ARCHIEVE VENDOR</p>
        </div>
    </header>
    <div class="body">
        <div class="content">
            <b>归档员欧尔(Filer Ool)</b
            >，又称<b>无尽档案塔商人</b>，是<b>无尽档案塔</b>的特殊行商之一。
            <br /><br />
            归档员欧尔所出售的商品需要使用<b>档案塔财富</b>来购买，且只有在完成一个循环之后才会出现。
            <br /><br />
            归档员欧尔的货物会在GMT标准时间周三凌晨0点、<b>北京时间每周三上午八点</b>准时刷新，持续一周。
            <br /><br />
            归档员欧尔会出售包括篆刻脚本、技能样式碎片、书籍、样式书在内的特殊物品。
        </div>
        <h2>档案塔商品列表（{{ time }}）</h2>
                    ...
                    >
                        ...
                        <td>
                            <span v-if="item.value" class="infinite-gold-column">
                                {{ item.value }} 档案塔财富
                                <img
                                    src="/public/icon/Archival_Fortunes.png"
                                    class="goldicon"
                                    alt="Gold Icon"
                                />
                            </span>
        ...
        const fetchInfinite = async () => {
            try {
                const response = await fetch(
                    "http://localhost:3000/api/infinite-vendor"
                );
                if (!response.ok) {
